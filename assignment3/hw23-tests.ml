...
;
  ...
  ("1234", ScmNumber (ScmRational (1234, 1)), ScmConst (ScmNumber (ScmRational (1234, 1))),
   ScmConst' (ScmNumber (ScmRational (1234, 1))))
   ...
   ScmPair
    (ScmPair
      (ScmPair
       ...
       ScmNil),
     ScmPair
      (ScmPair (ScmSymbol "fact", ScmPair (ScmNumber (ScmRational (5, 1)), ScmNil)),
       ScmNil))),
 ScmApplic
    ...
    ScmSeq'
     [ScmVarSet' (Var' ("fact", Param 0), ScmBox' (Var' ("fact", Param 0)));
      ...
      ScmApplic' (ScmBoxGet' (Var' ("fact", Param 0)),
       [ScmConst' (ScmNumber (ScmRational (5, 1)))], Tail_Call)]),
...
("(let* ((a 1) (b 2) (a (+ a b)) (b (* a b))) (list a b))",
 ScmPair
  (ScmSymbol "let*",
   ScmPair
    (ScmPair
      (ScmPair (ScmSymbol "a", ScmPair (ScmNumber (ScmRational (1, 1)), ScmNil)),
       ScmPair
     ...
     (ScmLambda' (["b"], Simple,
       ScmApplic'
        ...
        [ScmApplic' (ScmVarGet' (Var' ("+", Free)),
          ...
          Non_Tail_Call)],
        Tail_Call)),
     [ScmConst' (ScmNumber (ScmRational (2, 1)))], Tail_Call)),
  [ScmConst' (ScmNumber (ScmRational (1, 1)))], Non_Tail_Call))
...
("(let ((a 1) (b 2) (c 3)) (+ a b c))",
      ...
      (ScmPair (ScmSymbol "a", ScmPair (ScmNumber (ScmRational (1, 1)), ScmNil)),
       ScmPair
        (ScmPair (ScmSymbol "b", ScmPair (ScmNumber (ScmRational (2, 1)), ScmNil)),
         ScmPair
          (ScmPair
            (ScmSymbol "c", ScmPair (ScmNumber (ScmRational (3, 1)), ScmNil)),
           ScmNil))),
     ScmPair
      (ScmPair
        (ScmSymbol "+",
 ...
 ScmApplic
  (ScmLambda (["a"; "b"; "c"], Simple,
    ScmApplic (ScmVarGet (Var "+"),
     [ScmVarGet (Var "a"); ScmVarGet (Var "b"); ScmVarGet (Var "c")])),
  [ScmConst (ScmNumber (ScmRational (1, 1))); ScmConst (ScmNumber (ScmRational (2, 1)));
...
let second = (fun (a,b,c,d) -> b);;
let third = (fun (a,b,c,d) -> c);;
let fourth = (fun (a,b,c,d) -> d);;
let test_tp ind = (third(List.nth quadruples ind) = (Tag_Parser.tag_parse(second(List.nth quadruples ind))));;
let test_sa ind = (fourth (List.nth quadruples ind) = (Semantic_Analysis.semantics (third(List.nth quadruples ind))));;
