...
module Tag_Parser : TAG_PARSER = struct
  ...
  let rec letrec_ribs ribs = match ribs with
      | ScmPair (ScmPair(foo,_), foos) -> ScmPair (ScmPair (foo, ScmPair
                                          (ScmPair (ScmSymbol "quote", ScmPair 
                                          (ScmSymbol "whatever", ScmNil)),
                                          ScmNil)), letrec_ribs foos)
      |ScmNil -> ScmNil
      | _ -> raise (X_this_should_not_happen "letrec_ribs");;
  
  let rec letrec_body expers ribs = match ribs with
    | ScmPair(rib, rest) -> 
      ScmPair (ScmPair (ScmSymbol "set!",rib), letrec_body expers rest)
    | ScmNil -> expers
    | _ -> raise (X_this_should_not_happen "letrec_body");;

  let rec vars_exprs = function
    | ScmNil -> ([], [])
    | ScmPair (ScmPair (var, ScmPair (expr, ScmNil)), ribs) ->
        let (vars, exprs) = vars_exprs ribs in
        (var :: vars, expr :: exprs)
    | _ -> raise (X_syntax "malformed let-ribs");;

  let rec tag_parse sexpr =
    match sexpr with
    |ScmVector _ | ScmVoid | ScmBoolean _ | ScmChar _ | ScmString _ | ScmNumber _ ->
       ScmConst sexpr
    | ScmPair (ScmSymbol "quote", ScmPair (sexpr, ScmNil)) ->
       ScmConst sexpr
    ...
    | ScmPair (ScmSymbol "let", ScmPair (ribs, exprs)) ->
      let (vars, es) = vars_exprs ribs in
      let vars = scheme_sexpr_list_of_sexpr_list vars in
      let es = scheme_sexpr_list_of_sexpr_list es in
      tag_parse
        (ScmPair (ScmPair (ScmSymbol "lambda",
                            ScmPair (vars, exprs)),
                  es))
  ...
  | ScmApplic' (proc, args, app_kind) ->
     ...
     let args =
       Reader.scheme_sexpr_list_of_sexpr_list
         (List.map sexpr_of_expr' args) in
     ScmPair (proc, args)
  | _ -> raise (X_this_should_not_happen "sexpr_of_expr");;
